...
#define WAKEBYTE_TIMEOUT_MSEC	(80)
...
void geni_se_dump_dbg_regs(struct uart_port *uport)
{
	...
	unsigned char __iomem *base = uport->membase;
...
{
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	unsigned char __iomem *membase = uport->membase;
	...
	if (enable) {
		geni_write_reg(port->ser_clk_cfg, membase, GENI_SER_M_CLK_CFG);
		geni_write_reg(port->ser_clk_cfg, membase, GENI_SER_S_CLK_CFG);
	} else {
		geni_write_reg(0x1, membase, GENI_SER_M_CLK_CFG);
		geni_write_reg(0x1, membase, GENI_SER_S_CLK_CFG);
	}
...
static void msm_geni_serial_enable_interrupts(struct uart_port *uport)
{
	...
	geni_m_irq_en = geni_read_reg(membase, SE_GENI_M_IRQ_EN);
	geni_s_irq_en = geni_read_reg(membase, SE_GENI_S_IRQ_EN);
	...
	geni_write_reg(geni_m_irq_en, membase, SE_GENI_M_IRQ_EN);
	geni_write_reg(geni_s_irq_en, membase, SE_GENI_S_IRQ_EN);
	if (port->xfer_mode == GENI_SE_DMA) {
		geni_write_reg(DMA_TX_IRQ_BITS, membase, SE_DMA_TX_IRQ_EN_SET);
		geni_write_reg(DMA_RX_IRQ_BITS, membase, SE_DMA_RX_IRQ_EN_SET);
	}

	if (!uart_console(uport)) {
		geni_m_irq_en = geni_read_reg(membase, SE_GENI_M_IRQ_EN);
		geni_s_irq_en = geni_read_reg(membase, SE_GENI_S_IRQ_EN);
		geni_dma_tx_irq_en = geni_read_reg(membase, SE_DMA_TX_IRQ_EN);
		geni_dma_rx_irq_en = geni_read_reg(membase, SE_DMA_RX_IRQ_EN);
		UART_LOG_DBG(port->ipc_log_irqstatus, uport->dev,
...
static bool msm_serial_try_disable_interrupts(struct uart_port *uport)
{
	...
	geni_write_reg(geni_s_irq_en, membase, SE_GENI_S_IRQ_EN);
	if (port->xfer_mode == GENI_SE_DMA) {
		geni_write_reg(DMA_TX_IRQ_BITS, membase, SE_DMA_TX_IRQ_EN_CLR);
		geni_write_reg(DMA_RX_IRQ_BITS, membase, SE_DMA_RX_IRQ_EN_CLR);
	}
...
static bool geni_wait_for_cmd_done(struct uart_port *uport, bool is_irq_masked)
{
	...
	if (is_irq_masked) {
		 ...
		 */
		if (port->m_cmd) {
			while (!port->m_cmd_done && timeout > 0) {
				msm_geni_serial_handle_isr(uport, &flags, true);
				timeout--;
				udelay(100);
			}
		} else if (port->s_cmd) {
			while (!port->s_cmd_done && timeout > 0) {
				msm_geni_serial_handle_isr(uport, &flags, true);
				timeout--;
				udelay(100);
			}
 ...
 */
static ssize_t capture_kpi_store(struct device *dev, struct device_attribute *attr,
 ...
 * Return: Size copied in the buffer
 */
static ssize_t hs_uart_operation_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_geni_serial_port *port = platform_get_drvdata(pdev);
 ...
 * Return: Size copied in the buffer
 */
static ssize_t hs_uart_operation_store(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t size)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_geni_serial_port *port = platform_get_drvdata(pdev);
 ...
 * Return: Size copied in the buffer
 */
static ssize_t hs_uart_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_geni_serial_port *port = platform_get_drvdata(pdev);
...

{
	...
	hex_dump_to_buffer(string, len, DATA_BYTES_PER_LINE, 1, buf, sizeof(buf), false);
...
static bool check_transfers_inflight(struct uart_port *uport)
{
	...
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	unsigned char __iomem *membase = uport->membase;
	u32 geni_status = geni_read_reg(membase, SE_GENI_STATUS);
	...
	if (port->xfer_mode == GENI_SE_DMA) {
		...
		rx_fifo_status = geni_read_reg(membase, SE_DMA_RX_LEN_IN);
	} else {
		tx_fifo_status = geni_read_reg(membase, SE_GENI_TX_FIFO_STATUS);
		rx_fifo_status = geni_read_reg(membase, SE_GENI_RX_FIFO_STATUS);
	}
...
static int wait_for_transfers_inflight(struct uart_port *uport)
{
	...
	geni_status = geni_read_reg(membase, SE_GENI_STATUS);
	...
	while (iter < WAIT_XFER_MAX_ITER) {
		if (check_transfers_inflight(uport)) {
			usleep_range(WAIT_XFER_MIN_TIMEOUT_US, WAIT_XFER_MAX_TIMEOUT_US);
			iter++;
		} else {
			break;
		}
	...
	if (check_transfers_inflight(uport)) {
		rx_len_in = geni_read_reg(membase, SE_DMA_RX_LEN_IN);
		if (rx_len_in) {
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
...
static unsigned int msm_geni_serial_get_mctrl(struct uart_port *uport)
{
	...
	geni_ios = geni_read_reg(membase, SE_GENI_IOS);
...
static void msm_geni_cons_set_mctrl(struct uart_port *uport, unsigned int mctrl)
{
}
 ...
 * Return: None
 */
static void msm_geni_serial_set_mctrl(struct uart_port *uport, unsigned int mctrl)
{
	...
	geni_write_reg(uart_manual_rfr, membase, SE_UART_MANUAL_RFR);
...
static struct msm_geni_serial_port *get_port_from_line(int line, bool is_console)
{
...
static int msm_geni_serial_poll_bit(struct uart_port *uport, int offset, int bit_field, bool set)
{
	int iter = 0;
	unsigned int reg;
	bool met = false;
...
static void msm_geni_serial_setup_tx(struct uart_port *uport, unsigned int xmit_size)
{
	u32 m_cmd = 0;
	unsigned char __iomem *membase = uport->membase;

	geni_write_reg(xmit_size, membase, SE_UART_TX_TRANS_LEN);
	m_cmd |= (UART_START_TX << M_OPCODE_SHFT);
	geni_write_reg(m_cmd, membase, SE_GENI_M_CMD0);
	/*
...
static void msm_geni_serial_poll_tx_done(struct uart_port *uport)
{
	...
	done = msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS, M_CMD_DONE_EN, true);
	if (!done) {
		/*
		 * Failure IPC logs are not added as this API is
		 * used by early console and it doesn't have log handle.
		 */
		geni_write_reg(M_GENI_CMD_CANCEL, membase, SE_GENI_M_CMD_CTRL_REG);
		done = msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS, M_CMD_CANCEL_EN, true);
		if (!done) {
			geni_write_reg(M_GENI_CMD_ABORT, membase, SE_GENI_M_CMD_CTRL_REG);
			msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS, M_CMD_ABORT_EN, true);
		}
	}
	irq_clear = geni_read_reg(membase, SE_GENI_M_IRQ_STATUS);
	geni_write_reg(irq_clear, membase, SE_GENI_M_IRQ_CLEAR);
}
...
static int msm_geni_serial_get_char(struct uart_port *uport)
{
	...
	if (!(msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS, M_SEC_IRQ_EN, true)))
		return -ENXIO;

	m_irq_status = geni_read_reg(membase, SE_GENI_M_IRQ_STATUS);
	s_irq_status = geni_read_reg(membase, SE_GENI_S_IRQ_STATUS);
	geni_write_reg(m_irq_status, membase, SE_GENI_M_IRQ_CLEAR);
	geni_write_reg(s_irq_status, membase, SE_GENI_S_IRQ_CLEAR);
	 ...
	 */
	mb();
	rx_fifo = geni_read_reg(membase, SE_GENI_RX_FIFOn);
	rx_fifo &= 0xFF;
	return rx_fifo;
}

static void msm_geni_serial_poll_put_char(struct uart_port *uport, unsigned char c)
{
	int b = (int) c;
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	unsigned char __iomem *membase = uport->membase;

	geni_write_reg(port->tx_wm, membase, SE_GENI_TX_WATERMARK_REG);
	msm_geni_serial_setup_tx(uport, 1);
	if (!msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS,
				M_TX_FIFO_WATERMARK_EN, true))
		WARN_ON(1);
	geni_write_reg(b, membase, SE_GENI_TX_FIFOn);
	geni_write_reg(M_TX_FIFO_WATERMARK_EN, membase, SE_GENI_M_IRQ_CLEAR);
	/*
...
static void
__msm_geni_serial_console_write(struct uart_port *uport, const char *s, unsigned int count)
{
	int new_line = 0;
	int i;
	...
	geni_write_reg(tx_wm, membase, SE_GENI_TX_WATERMARK_REG);
	...
	while (i < count) {
		...
		while (!msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS,
						 M_TX_FIFO_WATERMARK_EN, true))
			break;
		chars_to_write = min((unsigned int)(count - i), avail_fifo_bytes);
		if ((chars_to_write << 1) > avail_fifo_bytes)
			chars_to_write = (avail_fifo_bytes >> 1);
		uart_console_write(uport, (s + i), chars_to_write, msm_geni_serial_wr_char);
		geni_write_reg(M_TX_FIFO_WATERMARK_EN, membase, SE_GENI_M_IRQ_CLEAR);
...
static void msm_geni_serial_console_write(struct console *co, const char *s, unsigned int count)
{
	struct uart_port *uport;
	...
	geni_status = readl_relaxed(membase + SE_GENI_STATUS);
	...
	if ((geni_status & M_GENI_CMD_ACTIVE) && !locked) {
		...
		if (timeout) {
			IPC_LOG_MSG(port->console_log,
				    ...
				    __func__, geni_read_reg(membase, SE_GENI_STATUS));
			geni_se_dump_dbg_regs(uport);
			reinit_completion(&port->m_cmd_timeout);
			...
			if (timeout) {
				...
				geni_read_reg(membase, SE_GENI_STATUS));
				geni_se_dump_dbg_regs(uport);
			}

			msm_geni_serial_allow_rx(port);
			geni_write_reg(FORCE_DEFAULT, membase, GENI_FORCE_DEFAULT_REG);
		}
		...
		if (uart_circ_chars_pending(&uport->state->xmit)) {
			irq_en = geni_read_reg(membase, SE_GENI_M_IRQ_EN);
			geni_write_reg(irq_en | M_TX_FIFO_WATERMARK_EN, membase, SE_GENI_M_IRQ_EN);
		}
	}
...
static int handle_rx_console(struct uart_port *uport, unsigned int rx_fifo_wc,
			     unsigned int rx_last_byte_valid, unsigned int rx_last, bool drop_rx)
{
	...
	for (i = 0; i < rx_fifo_wc; i++) {
		...
		*port->rx_fifo = geni_read_reg(membase, SE_GENI_RX_FIFOn);
		if (drop_rx)
			continue;
		rx_char = (unsigned char *)port->rx_fifo;
...
static void msm_geni_uart_ev_cb(struct dma_chan *ch, struct msm_gpi_cb const *cb_str, void *ptr)
{
	struct msm_geni_serial_port *port = ptr;
	...
	if (cb_str->cb_event != MSM_GPI_QUP_NOTIFY)
		UART_LOG_DBG(port->ipc_log_misc, port->uport.dev,
			     ...
			     cb_str->error_log.error_code, geni_stat, cb_str->cb_event);
exit_ev_cb:
	UART_LOG_DBG(port->ipc_log_misc, port->uport.dev,
		     "%s: End\n", __func__);
}
...
{
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	struct msm_gpi_tre *tx_cfg0 = &port->gsi->tx_cfg0_t;
	struct msm_gpi_tre *rx_cfg0 = &port->gsi->rx_cfg0_t;
	...
	tx_cfg0->dword[0] = MSM_GPI_UART_CONFIG0_TRE_DWORD0(1, 0, flags, 4,
	...
	/* config0 */
	rx_cfg0->dword[0] = MSM_GPI_UART_CONFIG0_TRE_DWORD0(1, 0, flags, 4, stop_bit_len,
							    char_size);
	rx_cfg0->dword[1] = MSM_GPI_UART_CONFIG0_TRE_DWORD1(rfr_lvl, STALE_COUNT);
...
static void msm_geni_uart_gsi_tx_cb(void *ptr)
{
	...
	struct msm_geni_serial_port *port = tx_cb->userdata;
	...
	geni_se_tx_dma_unprep(&port->se, port->tx_dma, port->xmit_size);
	...
	UART_LOG_DBG(port->ipc_log_misc, port->uport.dev, "%s:End\n",
		     __func__);
}
...
static void msm_geni_uart_rx_queue_dma_tre(int index, struct uart_port *uport)
{
	...
	sg_set_buf(&rx_sg, &port->gsi->rx_t[index], sizeof(port->gsi->rx_t[index]));
	port->gsi->rx_desc = dmaengine_prep_slave_sg(port->gsi->rx_c, &rx_sg, 1, DMA_DEV_TO_MEM,
						     (DMA_PREP_INTERRUPT | DMA_CTRL_ACK));
	if (!port->gsi->rx_desc) {
		dev_err(uport->dev, "%s:Prep_slave_sg failed\n", __func__);
		return;
	}
	port->gsi->rx_desc->callback = msm_geni_uart_gsi_rx_cb;
	port->gsi->rx_desc->callback_param = &port->gsi->rx_cb;

	rx_cookie = dmaengine_submit(port->gsi->rx_desc);
...
static void msm_geni_uart_gsi_rx_cb(void *ptr)
{
	...
	struct msm_geni_serial_port *port = rx_cb->userdata;
	struct uart_port *uport = &port->uport;
	...
	UART_LOG_DBG(port->ipc_log_rx, uport->dev,
		     "%s: Start\n", __func__);
	ret = tty_insert_flip_string(tport, (unsigned char *)(port->rx_gsi_buf[port->count]),
				     rx_bytes);
	if (ret != rx_bytes)
		UART_LOG_DBG(port->ipc_log_rx, uport->dev,
	...
	dump_ipc(uport, port->ipc_log_rx, "GSI Rx",
		 (char *)port->rx_gsi_buf[port->count], 0, rx_bytes);

	msm_geni_uart_rx_queue_dma_tre(port->count, uport);
	port->count = (port->count + 1) % 4;
	UART_LOG_DBG(port->ipc_log_rx, uport->dev,
		     "%s: End\n", __func__);
}
...
static int msm_geni_allocate_chan(struct uart_port *uport)
{
	...
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		     "%s: Start\n", __func__);
	if (!port->gsi->rx_c) {
		port->gsi->rx_c = dma_request_slave_channel(uport->dev, "rx");
		if (!port->gsi->rx_c) {
			dev_err(uport->dev, "%s:Failed to allocate RX slv ch\n", __func__);
			ret = -EIO;
			goto out;
		}
		port->gsi->rx_ev.init.callback = msm_geni_uart_ev_cb;
		port->gsi->rx_ev.init.cb_param = port;
		port->gsi->rx_ev.cmd = MSM_GPI_INIT;
		port->gsi->rx_c->private = &port->gsi->rx_ev;
		ret = dmaengine_slave_config(port->gsi->rx_c, NULL);
		if (ret) {
	...
	if (!port->gsi->tx_c) {
		port->gsi->tx_c = dma_request_slave_channel(uport->dev, "tx");
		...
		port->gsi->tx_ev.init.callback = msm_geni_uart_ev_cb;
		port->gsi->tx_ev.init.cb_param = port;
		port->gsi->tx_ev.cmd = MSM_GPI_INIT;
		port->gsi->tx_c->private = &port->gsi->tx_ev;
		ret = dmaengine_slave_config(port->gsi->tx_c, NULL);
		if (ret) {
...
{
	struct msm_geni_serial_port *port = container_of(work, struct msm_geni_serial_port,
							 tx_xfer_work);
	struct uart_port *uport = &port->uport;
	struct circ_buf *xmit = &uport->state->xmit;
	dma_cookie_t tx_cookie;
	struct msm_gpi_tre *go_t = &port->gsi->tx_go_t;
	struct device *tx_dev = port->wrapper_dev;
	unsigned int xmit_size;
	...
	if (!xmit_size || port->tx_dma)
		return;

	dump_ipc(uport, port->ipc_log_tx, "DMA Tx",
	...
	sg_set_buf(port->gsi->tx_sg, &port->gsi->tx_cfg0_t, sizeof(port->gsi->tx_cfg0_t));
	index++;
	...
	ret = geni_se_common_iommu_map_buf(tx_dev, &port->tx_dma, &xmit->buf[xmit->tail],
	...
	port->gsi->tx_t.dword[0] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD0(port->tx_dma);
	port->gsi->tx_t.dword[1] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD1(port->tx_dma);
	port->gsi->tx_t.dword[2] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD2(xmit_size);
	port->gsi->tx_t.dword[3] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD3(0, 0, 1, 0, 0);

	sg_set_buf(&port->gsi->tx_sg[index++], &port->gsi->tx_t, sizeof(port->gsi->tx_t));

	port->gsi->tx_desc = dmaengine_prep_slave_sg(port->gsi->tx_c, port->gsi->tx_sg,
						     3, DMA_MEM_TO_DEV,
						     (DMA_PREP_INTERRUPT | DMA_CTRL_ACK));
	if (!port->gsi->tx_desc) {
	...
	port->gsi->tx_desc->callback = msm_geni_uart_gsi_tx_cb;
	port->gsi->tx_desc->callback_param = &port->gsi->tx_cb;
	tx_cookie = dmaengine_submit(port->gsi->tx_desc);
	...
	timeout = wait_for_completion_timeout(&port->tx_xfer, msecs_to_jiffies
	...
	return;
exit_gsi_tx_xfer:
	geni_se_common_iommu_unmap_buf(tx_dev, &port->tx_dma, port->xmit_size, DMA_TO_DEVICE);
	msm_geni_deallocate_chan(uport);
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
...
static int msm_geni_uart_gsi_xfer_rx(struct uart_port *uport)
{
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	dma_cookie_t rx_cookie;
	struct msm_gpi_tre *go_t = &port->gsi->rx_go_t;
	struct device *rx_dev = port->wrapper_dev;
	int i, k, index = 0;
	...
	sg_set_buf(port->gsi->rx_sg, &port->gsi->rx_cfg0_t, sizeof(port->gsi->rx_cfg0_t));
	index++;
	...
	for (i = 0; i < NUM_RX_BUF; i++) {
		port->rx_gsi_buf[i] = geni_se_common_iommu_alloc_buf(rx_dev, &port->dma_addr[i],
								     DMA_RX_BUF_SIZE);
		if (IS_ERR_OR_NULL(port->rx_gsi_buf[i])) {
			for (k = i; k > 0; k--) {
				geni_se_common_iommu_free_buf(rx_dev, &port->dma_addr[k - 1],
							      port->rx_gsi_buf[k - 1],
							      DMA_RX_BUF_SIZE);
			}
			msm_geni_deallocate_chan(uport);
		...
		port->gsi->rx_t[i].dword[0] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD0(port->dma_addr[i]);
		port->gsi->rx_t[i].dword[1] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD1(port->dma_addr[i]);
		port->gsi->rx_t[i].dword[2] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD2(DMA_RX_BUF_SIZE);
		port->gsi->rx_t[i].dword[3] = MSM_GPI_DMA_W_BUFFER_TRE_DWORD3(0, 0, 1, 1, 1);
		sg_set_buf(&port->gsi->rx_sg[index++], &port->gsi->rx_t[i],
			   sizeof(port->gsi->rx_t[i]));
	}
	port->gsi->rx_desc = dmaengine_prep_slave_sg(port->gsi->rx_c, port->gsi->rx_sg,
						     6, DMA_DEV_TO_MEM,
						     (DMA_PREP_INTERRUPT | DMA_CTRL_ACK));
	if (!port->gsi->rx_desc) {
	...
	dma_async_issue_pending(port->gsi->rx_c);
	port->gsi_rx_done = true;

	return 0;
exit_gsi_xfer_rx:
	for (i = 0; i < NUM_RX_BUF; i++) {
		geni_se_common_iommu_free_buf(rx_dev, &port->dma_addr[i],
					      port->rx_gsi_buf[i], DMA_RX_BUF_SIZE);
	}
	msm_geni_deallocate_chan(uport);
	port->gsi_rx_done = false;
	return -EIO;
...
static int msm_geni_serial_prep_dma_tx(struct uart_port *uport)
{
	...
	start_time = geni_capture_start_time(&port->se, port->ipc_log_kpi,
					     __func__, port->uart_kpi);
	if (atomic_read(&port->flush_buffers))
		return -EIO;
	...
	if (port->uart_kpi) {
		port->uart_kpi_tx[port->kpi_idx].xfer_req_hw.len = port->xmit_size;
		port->uart_kpi_tx[port->kpi_idx].xfer_req_hw.time_stamp = sched_clock();
		port->kpi_idx++;
		if (port->kpi_idx >= UART_KPI_TX_RX_INSTANCES)
			port->kpi_idx = 0;
	}

	msm_geni_serial_setup_tx(uport, xmit_size);
	ret = geni_se_tx_dma_prep(&port->se, &xmit->buf[xmit->tail], xmit_size, &port->tx_dma);
	...
	if (!ret) {
		...
		msm_geni_update_uart_error_code(port, UART_ERROR_TX_DMA_MAP_FAIL);

		geni_write_reg(0, membase, SE_UART_TX_TRANS_LEN);
		port->m_cmd_done = false;
		port->m_cmd = true;
		reinit_completion(&port->m_cmd_timeout);
		...
		if (port->xfer_mode == GENI_SE_DMA) {
			dma_dbg = geni_read_reg(membase, SE_DMA_DEBUG_REG0);
			if (dma_dbg & DMA_TX_ACTIVE) {
				port->m_cmd_done = false;
				reinit_completion(&port->m_cmd_timeout);
				geni_write_reg(1, membase, SE_DMA_TX_FSM_RST);
				...
				if (timeout) {
					...
					msm_geni_update_uart_error_code
							(port, UART_ERROR_TX_FSM_RESET_FAIL);
					geni_se_dump_dbg_regs(uport);
				} else {
	...
	geni_capture_stop_time(&port->se, port->ipc_log_kpi, __func__,
			       port->uart_kpi, start_time, port->xmit_size, port->cur_baud);
	return ret;
}
...
static void msm_geni_serial_start_tx(struct uart_port *uport)
{
	...
	if (uart_console(uport) && (uport->suspended || atomic_read(&port->is_clock_off))) {
		IPC_LOG_MSG(port->console_log, "%s. Console in suspend state\n", __func__);
		return;
	}
	 ...
	 */
	if (atomic_read(&port->flush_buffers))
		atomic_set(&port->flush_buffers, 0);
	...
	if (port->xfer_mode == GENI_SE_FIFO) {
		...
		if (port->uart_kpi) {
			port->uart_kpi_tx[port->kpi_idx].xfer_req_sw.len = port->xmit_size;
			port->uart_kpi_tx[port->kpi_idx].xfer_req_sw.time_stamp = sched_clock();
		}
		if (msm_geni_serial_prep_dma_tx(uport) == -EPERM) {
			UART_LOG_DBG(port->ipc_log_tx, uport->dev, "%s: tx_en=0,\n",
				     __func__);
			goto exit_start_tx;
		}
...
check_flow_ctrl:
	...
	if (++ios_log_limit % 5 == 0) {
		UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s: ios: 0x%x\n",
			     __func__, geni_ios);
		ios_log_limit = 0;
	}
	geni_capture_stop_time(&port->se, port->ipc_log_kpi, __func__,
...
static void start_rx_sequencer(struct uart_port *uport)
{
	...
	if (geni_status & S_GENI_CMD_ACTIVE) {
		if (port->xfer_mode == GENI_SE_DMA) {
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
				     ...
				     __func__, geni_status);
			geni_se_common_rx_dma_start(membase, DMA_RX_BUF_SIZE, &port->rx_dma);
		}
	...
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		     ...
		     geni_status, geni_read_reg(membase, SE_DMA_DEBUG_REG0));
}
...
static void msm_geni_serial_set_manual_flow(bool enable, struct msm_geni_serial_port *port)
{
	u32 uart_manual_rfr = 0;

	if (!enable) {
		uart_manual_rfr |= (UART_MANUAL_RFR_EN);
		geni_write_reg(uart_manual_rfr, port->uport.membase, SE_UART_MANUAL_RFR);
		...
		geni_write_reg(0, port->uport.membase, SE_UART_MANUAL_RFR);
		/* Ensure that the manual flow off writes go through */
		mb();
		uart_manual_rfr = geni_read_reg(port->uport.membase, SE_UART_MANUAL_RFR);
		UART_LOG_DBG(port->ipc_log_misc, port->uport.dev,
...
static int stop_rx_sequencer(struct uart_port *uport)
{
	...
	if (!uart_console(uport)) {
		...
		dma_rx_status = geni_read_reg(membase, SE_DMA_RX_IRQ_STAT);
	...
	if (timeout || is_rx_active) {
		...
		if (s_irq_status & S_CMD_CANCEL_EN) {
			...
			geni_write_reg(S_CMD_CANCEL_EN, membase, SE_GENI_S_IRQ_CLEAR);
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		...
		if (port->xfer_mode == GENI_SE_DMA) {
			port->s_cmd_done = false;
			reinit_completion(&port->s_cmd_timeout);
			geni_write_reg(1, membase, SE_DMA_RX_FSM_RST);
			...
			unsigned int rx_last_byte_valid, unsigned int rx_last, bool drop_rx)
{
	...
	rx_bytes = (port->tx_fifo_width * (rx_fifo_wc - 1)) >> 3;
	rx_bytes += ((rx_last && rx_last_byte_valid) ?
		     rx_last_byte_valid : port->tx_fifo_width >> 3);

	tport = &uport->state->port;
	ioread32_rep((membase + SE_GENI_RX_FIFOn), port->rx_fifo, rx_fifo_wc);
	if (drop_rx)
		return 0;
...
static int msm_geni_serial_handle_rx(struct uart_port *uport, bool drop_rx)
{
	...
	if (rx_fifo_wc)
		ret = port->handle_rx(uport, rx_fifo_wc, rx_last_byte_valid, rx_last, drop_rx);
	return ret;
}

static int msm_geni_serial_handle_tx(struct uart_port *uport, bool done, bool active)
{
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	struct circ_buf *xmit = &uport->state->xmit;
	unsigned char __iomem *membase = uport->membase;
	...
	unsigned int xmit_size;
	unsigned int fifo_width_bytes = (uart_console(uport) ? 1 : (port->tx_fifo_width >> 3));
	int temp_tail = 0;
	int irq_en;
	...
	pending = active ? port->cur_tx_remaining : uart_circ_chars_pending(xmit);

	/* All data has been transmitted and acknowledged as received */
	if (!pending && !tx_fifo_status && done)
		goto exit_handle_tx;

	avail_fifo_bytes = port->tx_fifo_depth - (tx_fifo_status & TX_FIFO_WC);
	...
	while (i < xmit_size) {
		...
		geni_write_reg(buf, membase, SE_GENI_TX_FIFOn);
...
static int msm_geni_serial_handle_dma_rx(struct uart_port *uport, bool drop_rx)
{
	...
	if (unlikely(!port->rx_buf)) {
		UART_LOG_DBG(port->ipc_log_rx, uport->dev, "%s: NULL Rx_buf\n", __func__);
		return 0;
	}

	rx_bytes = geni_read_reg(membase, SE_DMA_RX_LEN_IN);
	if (unlikely(!rx_bytes)) {
		UART_LOG_DBG(port->ipc_log_rx, uport->dev, "%s: Size %d\n", __func__, rx_bytes);
		return 0;
	}
	...
	if (ret != rx_bytes) {
		UART_LOG_DBG(port->ipc_log_rx, uport->dev,
			     "%s: ret %d rx_bytes %d\n", __func__, ret, rx_bytes);
		rx_buf = (unsigned char *)(port->rx_buf);
		rx_buf += ret;
	 ...
	 */
	memset(port->rx_buf, 0, rx_bytes_copied);

	if (port->uart_error == UART_ERROR_RX_PARITY_ERROR ||
	    port->uart_error == UART_ERROR_RX_BREAK_ERROR ||
	    port->uart_error == UART_ERROR_RX_FRAMING_ERR)
		msm_geni_update_uart_error_code(port, UART_ERROR_DEFAULT);

	geni_capture_stop_time(&port->se, port->ipc_log_kpi, __func__,
			       port->uart_kpi, start_time, rx_bytes, port->cur_baud);
...
static int msm_geni_serial_handle_dma_tx(struct uart_port *uport)
{
	...
	if (port->uart_kpi) {
		port->uart_kpi_tx[port->kpi_comp_idx].xfer_req_comp.len = port->xmit_size;
		port->uart_kpi_tx[port->kpi_comp_idx].xfer_req_comp.time_stamp = sched_clock();
		len = port->uart_kpi_tx[port->kpi_comp_idx].xfer_req_comp.len;
		comp_time = port->uart_kpi_tx[port->kpi_comp_idx].xfer_req_comp.time_stamp;
		sw_time = port->uart_kpi_tx[port->kpi_comp_idx].xfer_req_sw.time_stamp;
		exec_time = comp_time - sw_time;
		UART_LOG_DBG(port->ipc_log_kpi, uport->dev,
			     "%s:TX transfer time %llu nsec(%llu usec) for %d bytes with freq %d index:%d\n",
			     __func__, exec_time, (exec_time / 1000), len, port->cur_baud,
			     port->kpi_comp_idx);
		sw_time = port->uart_kpi_tx[port->kpi_comp_idx].xfer_req_hw.time_stamp;
		exec_time = comp_time - sw_time;
		UART_LOG_DBG(port->ipc_log_kpi, uport->dev,
			     ...
			     port->kpi_comp_idx);
		port->kpi_comp_idx++;
		if (port->kpi_comp_idx >= UART_KPI_TX_RX_INSTANCES)
			port->kpi_comp_idx = 0;
	}
...
static bool handle_tx_fifo_xfer(u32 m_irq_status, struct uart_port *uport)
{
	...
	unsigned char __iomem *membase = uport->membase;
	u32 geni_status = geni_read_reg(membase, SE_GENI_STATUS);
	u32 m_irq_en = geni_read_reg(membase, SE_GENI_M_IRQ_EN);

	if ((m_irq_status & m_irq_en) & (M_TX_FIFO_WATERMARK_EN | M_CMD_DONE_EN))
		msm_geni_serial_handle_tx(uport, m_irq_status & M_CMD_DONE_EN,
				...
				unsigned long *flags, bool is_irq_masked)
{
	...
	if (s_irq_status & (S_GP_IRQ_0_EN | S_GP_IRQ_1_EN)) {
		if (s_irq_status & S_GP_IRQ_0_EN)
			uport->icount.parity++;
		UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s.sirq 0x%x parity:%d\n",
			     __func__, s_irq_status, uport->icount.parity);
		drop_rx = true;
	} else if (s_irq_status & (S_GP_IRQ_2_EN | S_GP_IRQ_3_EN)) {
		uport->icount.brk++;
		UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s.sirq 0x%x break:%d\n",
...
static bool handle_tx_dma_xfer(u32 m_irq_status, struct uart_port *uport)
{
	...
	unsigned char __iomem *membase = uport->membase;
	bool ret = false;
	u32 dma_tx_status = geni_read_reg(membase, SE_DMA_TX_IRQ_STAT);
	if (dma_tx_status) {
		geni_write_reg(dma_tx_status, membase, SE_DMA_TX_IRQ_CLR);
		if (dma_tx_status & (TX_RESET_DONE | TX_GENI_CANCEL_IRQ))
			return true;
		if (dma_tx_status & TX_DMA_DONE) {
			msm_geni_serial_handle_dma_tx(uport);
			if (port->uart_error == SOC_ERROR_START_TX_IOS_SOC_RFR_HIGH) {
...
static bool handle_rx_dma_xfer(u32 s_irq_status, struct uart_port *uport)
{
	...
	start_time = geni_capture_start_time(&port->se, port->ipc_log_kpi,
					     __func__, port->uart_kpi);
	spin_lock_irqsave(&port->rx_lock, lock_flags);
	dma_rx_status = geni_read_reg(membase, SE_DMA_RX_IRQ_STAT);
	if (dma_rx_status) {
		geni_write_reg(dma_rx_status, membase, SE_DMA_RX_IRQ_CLR);
		...
		if (dma_rx_status & UART_DMA_RX_PARITY_ERR) {
			uport->icount.parity++;
			msm_geni_update_uart_error_code(port, UART_ERROR_RX_PARITY_ERROR);
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		...
		if (dma_rx_status & UART_DMA_RX_FRAMING_ERR) {
			uport->icount.frame++;
			msm_geni_update_uart_error_code(port, UART_ERROR_RX_FRAMING_ERR);
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		...
		if (dma_rx_status & UART_DMA_RX_BREAK) {
			...
			msm_geni_update_uart_error_code(port, UART_ERROR_RX_BREAK_ERROR);
			geni_se_dump_dbg_regs(uport);
		}
		...
		if (dma_rx_status & RX_EOT || dma_rx_status & RX_DMA_DONE) {
			...
			if (!(dma_rx_status & RX_GENI_CANCEL_IRQ)) {
				UART_LOG_DBG(port->ipc_log_misc, uport->dev,
					     "%s mapping rx dma\n", __func__);
				geni_se_common_rx_dma_start(membase, DMA_RX_BUF_SIZE,
							    &port->rx_dma);
			} else {
				UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		...
		if (dma_rx_status & RX_SBE) {
			...
			msm_geni_update_uart_error_code(port, UART_ERROR_RX_SBE_ERROR);
		}
	...
	spin_unlock_irqrestore(&port->rx_lock, lock_flags);
	geni_capture_stop_time(&port->se, port->ipc_log_kpi, __func__,
			       port->uart_kpi, start_time, 0, 0);
	return ret;
}
...
static void msm_geni_serial_clear_irqs(struct uart_port *uport)
{
	...
	dma_tx_status = geni_read_reg(membase, SE_DMA_TX_IRQ_STAT);
	if (dma_tx_status)
		geni_write_reg(dma_tx_status, membase, SE_DMA_TX_IRQ_CLR);
...
static void msm_geni_serial_handle_isr(struct uart_port *uport, unsigned long *flags,
				       bool is_irq_masked)
{
	unsigned int m_irq_status;
	unsigned int s_irq_status;
	...
	start_time = geni_capture_start_time(&port->se, port->ipc_log_kpi,
					     __func__, port->uart_kpi);
	if (uart_console(uport) && atomic_read(&port->is_clock_off)) {
		IPC_LOG_MSG(port->console_log, "%s. Console in suspend state\n", __func__);
	...
	s_irq_status = geni_read_reg(membase, SE_GENI_S_IRQ_STATUS);
	if (uart_console(uport))
		IPC_LOG_MSG(port->console_log, "%s. sirq 0x%x mirq:0x%x\n",
			    __func__, s_irq_status, m_irq_status);
	geni_write_reg(m_irq_status, membase, SE_GENI_M_IRQ_CLEAR);
	geni_write_reg(s_irq_status, membase, SE_GENI_S_IRQ_CLEAR);
	if ((m_irq_status & M_ILLEGAL_CMD_EN)) {
		if (uart_console(uport))
			IPC_LOG_MSG(port->console_log, "%s Illegal interrupt sirq:0x%x mirq:0x%x\n",
				    __func__, s_irq_status, m_irq_status);
		else {
			msm_geni_update_uart_error_code(port, UART_ERROR_ILLEGAL_INTERRUPT);
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
	...
	if (s_irq_status & S_RX_FIFO_WR_ERR_EN) {
		...
		UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s.sirq 0x%x buf_overrun:%d\n",
			     __func__, s_irq_status, uport->icount.buf_overrun);
		msm_geni_update_uart_error_code(port, UART_ERROR_BUFFER_OVERRUN);
	}

	dma = geni_read_reg(membase, SE_GENI_DMA_MODE_EN);
	if (!dma) {
		m_cmd_done = handle_tx_fifo_xfer(m_irq_status, uport);
		s_cmd_done = handle_rx_fifo_xfer(s_irq_status, uport, flags, is_irq_masked);
	} else {
		dma_tx_status = geni_read_reg(membase, SE_DMA_TX_IRQ_STAT);
		dma_rx_status = geni_read_reg(membase, SE_DMA_RX_IRQ_STAT);
...
exit_geni_serial_isr:
	if (m_cmd_done) {
		port->m_cmd_done = true;
		complete(&port->m_cmd_timeout);
	}

	if (s_cmd_done) {
		port->s_cmd_done = true;
		complete(&port->s_cmd_timeout);
	}
}
...
static void msm_geni_wakeup_work(struct work_struct *work)
{
	...
	port = container_of(work, struct msm_geni_serial_port, wakeup_irq_dwork.work);
	if (!atomic_read(&port->check_wakeup_byte))
		return;
	uport = &port->uport;
...
static int get_tx_fifo_size(struct msm_geni_serial_port *port)
{
	...
	uport->fifosize = ((port->tx_fifo_depth * port->tx_fifo_width) >> 3);
	return 0;
}
	...
	if (uart_console(uport)) {
		...
		if (port->xfer_mode == GENI_GPI_DMA) {
			...
			UART_LOG_DBG(port->ipc_log_misc, uport->dev,
				     "%s: Stop Rx Engine\n", __func__);
			timeout = wait_for_completion_timeout
				(&port->xfer, msecs_to_jiffies(POLL_WAIT_TIMEOUT_MSEC));
			...
			if (port->gsi->rx_c) {
				...
				UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s:Unmap buf done\n",
					     __func__);
			}
			if (port->gsi->tx_c) {
				UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s:GSI DMA-Tx ch\n",
					     __func__);
				msm_geni_serial_stop_tx(uport);
				if (port->tx_dma) {
					geni_se_common_iommu_unmap_buf(tx_dev, &port->tx_dma,
								       port->xmit_size,
								       DMA_TO_DEVICE);
					UART_LOG_DBG(port->ipc_log_misc, uport->dev,
						     "%s:Unmap buf done\n", __func__);
		...
		if (!IS_ERR_OR_NULL(port->serial_rsc.geni_gpio_shutdown)) {
			ret = pinctrl_select_state(port->serial_rsc.geni_pinctrl,
						   port->serial_rsc.geni_gpio_shutdown);
			if (ret)
				UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		...
		port->uart_error = UART_ERROR_DEFAULT;
		atomic_set(&port->flush_buffers, 0);
		port->current_termios = NULL;
		port->count = 0;
	}
	port->port_state = UART_PORT_CLOSED_SHUTDOWN;
	geni_capture_stop_time(&port->se, port->ipc_log_kpi, __func__,
			       port->uart_kpi, start_time, 0, 0);
	UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s: End %d\n", __func__, ret);
}
...
static int msm_geni_serial_port_setup(struct uart_port *uport)
{
	...
	start_time = geni_capture_start_time(&port->se, port->ipc_log_kpi,
					     __func__, port->uart_kpi);
	set_rfr_wm(port);
	geni_write_reg(rxstale, membase, SE_UART_RX_STALE_CNT);
	if (port->gsi_mode) {
		port->xfer_mode = GENI_GPI_DMA;
	} else if (!uart_console(uport)) {
		/* For now only assume FIFO mode. */
		port->xfer_mode = GENI_SE_DMA;
		/* TX packing */
		geni_se_config_packing(&port->se, 8, 4, false, true, false);
		/* RX packing */
		geni_se_config_packing(&port->se, 8, 4, false, false, true);
		geni_write_reg(0x431c, membase, SE_GENI_CFG_REG80);
		if (!port->rx_fifo) {
			ret = -ENOMEM;
			goto exit_portsetup;
		}
		...
		if (!port->rx_buf) {
			port->rx_buf =
				geni_se_common_iommu_alloc_buf(port->wrapper_dev, &dma_address,
							       DMA_RX_BUF_SIZE);
			if (!port->rx_buf) {
				ret = -ENOMEM;
				goto exit_portsetup;
			}
			port->rx_dma = dma_address;
		}
	} else {
		/*
		...
		msm_geni_serial_poll_tx_done(uport);
		msm_geni_serial_enable_interrupts(uport);
		geni_se_config_packing(&port->se, 8, 1, false, true, false);
		geni_se_config_packing(&port->se, 8, 4, false, false, true);
	}
...
static int msm_geni_serial_startup(struct uart_port *uport)
{
	...
	if (likely(!uart_console(uport))) {
		...
		ret = msm_geni_serial_power_on(&port->uport);
		if (ret) {
	...
	port->port_state = UART_PORT_OPEN;

	geni_capture_stop_time(&port->se, port->ipc_log_kpi, __func__,
			       port->uart_kpi, start_time, 0, 0);
exit_startup:
	if (likely(!uart_console(uport)))
		msm_geni_serial_power_off(&port->uport);
	UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s: ret:%d\n", __func__, ret);
		...
		u32 rx_parity_cfg, u32 bits_per_char, u32 stop_bit_len)
{
	...
	geni_write_reg(loopback, membase, SE_UART_LOOPBACK_CFG);
	geni_write_reg(tx_trans_cfg, membase, SE_UART_TX_TRANS_CFG);
	geni_write_reg(tx_parity_cfg, membase, SE_UART_TX_PARITY_CFG);
	geni_write_reg(rx_trans_cfg, membase, SE_UART_RX_TRANS_CFG);
	geni_write_reg(rx_parity_cfg, membase, SE_UART_RX_PARITY_CFG);
	geni_write_reg(bits_per_char, membase, SE_UART_TX_WORD_LEN);
	geni_write_reg(bits_per_char, membase, SE_UART_RX_WORD_LEN);
	geni_write_reg(stop_bit_len, membase, SE_UART_TX_STOP_BIT_LEN);
	geni_se_config_packing(&port->se, bits_per_char, 4, false, true, false);
	geni_se_config_packing(&port->se, bits_per_char, 4, false, false, true);
}

static void msm_geni_serial_termios_cfg(struct uart_port *uport, struct ktermios *termios,
					int clk_div)
{
	u32 bits_per_char = 0;
	u32 stop_bit_len;
	unsigned char __iomem *membase = uport->membase;
	u32 tx_trans_cfg = geni_read_reg(membase, SE_UART_TX_TRANS_CFG);
	u32 tx_parity_cfg = geni_read_reg(membase, SE_UART_TX_PARITY_CFG);
	u32 rx_trans_cfg = geni_read_reg(membase, SE_UART_RX_TRANS_CFG);
	u32 rx_parity_cfg = geni_read_reg(membase, SE_UART_RX_PARITY_CFG);
	struct msm_geni_serial_port *port = GET_DEV_PORT(uport);
	...
	if (termios->c_cflag & CRTSCTS) {
		geni_write_reg(0x0, membase, SE_UART_MANUAL_RFR);
		UART_LOG_DBG(port->ipc_log_misc, uport->dev,
					    ...
					    struct ktermios *termios, unsigned int baud)
{
	 ...
	 * required frequency.
	 */
	ret = geni_se_clk_freq_match(&port->se, desired_rate, &clk_idx, &clk_rate, false);
	if (ret) {
					...
					struct ktermios *termios, const struct ktermios *old)
{
	...
	if (IS_ENABLED(CONFIG_SERIAL_MSM_GENI_HALF_SAMPLING) &&
		...
		geni_write_reg(0x21, membase, GENI_SER_M_CLK_CFG);
		geni_write_reg(0x21, membase, GENI_SER_S_CLK_CFG);
		geni_read_reg(membase, GENI_SER_M_CLK_CFG);
	}
...
static ssize_t ver_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct msm_geni_serial_port *port = platform_get_drvdata(pdev);
	ssize_t ret = 0;
...
static int msm_geni_console_setup(struct console *co, char *options)
{
	...
	if (msm_geni_serial_resources_on(port))
		WARN_ON(1);

	if (unlikely(get_se_proto(membase) != GENI_SE_UART)) {
		msm_geni_serial_resources_off(port);
		return -ENXIO;
	}
...
static void msm_geni_serial_debug_init(struct uart_port *uport, bool console)
{
	...
	port->dbg = debugfs_create_dir(dev_name(uport->dev), NULL);
	...
	if (!console) {
		...
		if (!port->ipc_log_rx) {
			...
			port->ipc_log_rx = ipc_log_context_create(IPC_LOG_TX_RX_PAGES, name, 0);
			if (!port->ipc_log_rx)
		...
		if (!port->ipc_log_tx) {
			...
			port->ipc_log_tx = ipc_log_context_create(IPC_LOG_TX_RX_PAGES, name, 0);
			if (!port->ipc_log_tx)
		...
		if (!port->ipc_log_pwr) {
			...
			port->ipc_log_pwr = ipc_log_context_create(IPC_LOG_PWR_PAGES, name, 0);
			if (!port->ipc_log_pwr)
		...
		if (!port->ipc_log_misc) {
			...
			port->ipc_log_misc = ipc_log_context_create(IPC_LOG_MISC_PAGES, name, 0);
			if (!port->ipc_log_misc)
				dev_info(uport->dev, "Err in Misc IPC Log\n");
		}
		memset(name, 0, sizeof(name));
		if (!port->ipc_log_irqstatus) {
			scnprintf(name, sizeof(name), "%s%s", dev_name(uport->dev), "_irqstatus");
			port->ipc_log_irqstatus =
					ipc_log_context_create(IPC_LOG_MISC_PAGES, name, 0);
			if (!port->ipc_log_irqstatus)
		...
		memset(name, 0, sizeof(name));
		if (!port->ipc_log_new) {
			scnprintf(name, sizeof(name), "%s%s", dev_name(uport->dev), "_new");
			port->ipc_log_new = ipc_log_context_create(IPC_LOG_MISC_PAGES, name, 0);
			if (!port->ipc_log_new)
		...
		memset(name, 0, sizeof(name));
		if (!port->console_log) {
			scnprintf(name, sizeof(name), "%s%s", dev_name(uport->dev), "_console");
			port->console_log = ipc_log_context_create(IPC_LOG_MISC_PAGES, name, 0);
			if (!port->console_log)
				dev_info(uport->dev, "Err in Misc IPC Log\n");
		}
	}
		...
		unsigned int new_state, unsigned int old_state)
{
	...
	if (new_state == UART_PM_STATE_ON && old_state == UART_PM_STATE_OFF) {
		msm_geni_serial_resources_on(port);
		msm_geni_enable_disable_se_clk(uport, true);
...
static void msm_geni_serial_init_gsi(struct uart_port *uport)
{
	...
	port->gsi_mode = geni_read_reg(uport->membase, GENI_IF_DISABLE_RO) & FIFO_IF_DISABLE;
	ret = geni_icc_disable(&port->se);
	if (ret) {
		UART_LOG_DBG(port->ipc_log_misc, port->uport.dev,
			     "%s: Error %d geni_icc_disable failed\n", __func__, ret);
	...
	if (port->gsi_mode) {
		...
		port->xfer_mode = GENI_GPI_DMA;
		port->tx_wq = alloc_workqueue("%s", WQ_UNBOUND | WQ_HIGHPRI, 1,
					      dev_name(uport->dev));
		port->rx_wq = alloc_workqueue("%s", WQ_UNBOUND | WQ_HIGHPRI, 1,
					      dev_name(uport->dev));
		INIT_WORK(&port->tx_xfer_work, msm_geni_uart_gsi_xfer_tx);
		INIT_WORK(&port->rx_cancel_work, msm_geni_uart_gsi_cancel_rx);
		INIT_WORK(&port->tx_cancel_work, msm_geni_uart_gsi_cancel_tx);
	}
}

static int msm_geni_se_clks_on_off(struct msm_geni_serial_port *port, bool clk_on)
{
	int ret = 0;
	...
	if (clk_on) {
		...
		geni_se_common_clks_on(port->serial_rsc.se_clk, port->serial_rsc.m_ahb_clk,
				       port->serial_rsc.s_ahb_clk);
		msm_geni_enable_disable_se_clk(&port->uport, true);
	} else {
		msm_geni_enable_disable_se_clk(&port->uport, false);
		geni_se_common_clks_off(port->serial_rsc.se_clk, port->serial_rsc.m_ahb_clk,
					port->serial_rsc.s_ahb_clk);
...
static int msm_geni_serial_get_ver_info(struct uart_port *uport)
{
	...
	if (!port->is_console) {
		...
		port->ser_clk_cfg = 0x21;
		msm_geni_se_clks_on_off(port, true);
	}

	/* Basic HW and FW info */
	if (unlikely(geni_se_common_get_proto(membase) != GENI_SE_UART)) {
		dev_err(uport->dev, "%s: Invalid FW %d loaded.\n",
			 __func__, geni_se_common_get_proto(membase));
		ret = -ENXIO;
		goto exit_ver_info;
	}

	port->serial_rsc.proto = GENI_SE_UART;
	port->ver_info.m_fw_ver = geni_se_common_get_m_fw(membase);
	port->ver_info.s_fw_ver = geni_se_common_get_s_fw(membase);
	scnprintf(fwver, len, "FW Ver:0x%x%x", port->ver_info.m_fw_ver, port->ver_info.s_fw_ver);

	hw_ver = geni_se_get_qup_hw_version(&port->se);
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		     "%s: FW Ver: %s HW Ver: 0x%x\n", __func__, fwver, hw_ver);

	geni_se_common_get_major_minor_num(hw_ver, &port->ver_info.hw_major_ver,
					   &port->ver_info.hw_minor_ver,
					   &port->ver_info.hw_step_ver);
...
exit_ver_info:
	if (!port->is_console)
		msm_geni_se_clks_on_off(port, false);
					   ...
					   struct msm_geni_serial_port *port)
{
	...
	if ((port->wakeup_irq < 0 && !port->is_console))
		dev_info(&pdev->dev, "No wakeup IRQ configured\n");

	port->serial_rsc.geni_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR_OR_NULL(port->serial_rsc.geni_pinctrl)) {
		dev_err(&pdev->dev, "No pinctrl config specified!\n");
		return PTR_ERR(port->serial_rsc.geni_pinctrl);
	}

	if (!port->is_console) {
		if (IS_ERR_OR_NULL(pinctrl_lookup_state(port->serial_rsc.geni_pinctrl,
							PINCTRL_SHUTDOWN))) {
			dev_info(&pdev->dev, "No Shutdown config specified\n");
		} else {
			port->serial_rsc.geni_gpio_shutdown =
			pinctrl_lookup_state(port->serial_rsc.geni_pinctrl, PINCTRL_SHUTDOWN);
		}
	}

	port->serial_rsc.geni_gpio_active =
		pinctrl_lookup_state(port->serial_rsc.geni_pinctrl, PINCTRL_ACTIVE);
	...
	if (IS_ERR_OR_NULL(port->serial_rsc.geni_gpio_active)) {
		 ...
		 */
		port->serial_rsc.geni_gpio_active =
			pinctrl_lookup_state(port->serial_rsc.geni_pinctrl, PINCTRL_DEFAULT);
		if (IS_ERR_OR_NULL(port->serial_rsc.geni_gpio_active)) {
			dev_err(&pdev->dev, "No default config specified!\n");
			return PTR_ERR(port->serial_rsc.geni_gpio_active);
		}
	}

	port->serial_rsc.geni_gpio_sleep = pinctrl_lookup_state(port->serial_rsc.geni_pinctrl,
								PINCTRL_SLEEP);
	if (IS_ERR_OR_NULL(port->serial_rsc.geni_gpio_sleep)) {
		dev_err(&pdev->dev, "No sleep config specified!\n");
		return PTR_ERR(port->serial_rsc.geni_gpio_sleep);
	}
	...
	port->name = devm_kasprintf(uport->dev, GFP_KERNEL, "msm_serial_geni%d", uport->line);
	...
	if (port->wakeup_irq > 0) {
		port->wakeup_irq_wq = alloc_workqueue("%s", WQ_UNBOUND | WQ_HIGHPRI, 1,
						      dev_name(uport->dev));
		...
		INIT_DELAYED_WORK(&port->wakeup_irq_dwork, msm_geni_wakeup_work);
		irq_set_status_flags(port->wakeup_irq, IRQ_NOAUTOEN);
		ret = devm_request_irq(uport->dev, port->wakeup_irq, msm_geni_wakeup_isr,
...
static int msm_geni_serial_get_clk(struct platform_device *pdev, struct msm_geni_serial_port *port)
{
	int ret = 0;
	struct device *dev = &pdev->dev;

	port->serial_rsc.se_clk = devm_clk_get(&pdev->dev, "se-clk");
	if (IS_ERR(port->serial_rsc.se_clk)) {
		ret = PTR_ERR(port->serial_rsc.se_clk);
		dev_err(&pdev->dev, "Err getting SE Core clk %d\n", ret);
		msm_geni_update_uart_error_code(port, UART_ERROR_CLK_GET_FAIL);
		return ret;
	}

	port->serial_rsc.m_ahb_clk = devm_clk_get(dev->parent, "m-ahb");
	if (IS_ERR(port->serial_rsc.m_ahb_clk)) {
		ret = PTR_ERR(port->serial_rsc.m_ahb_clk);
		dev_err(&pdev->dev, "Err getting M AHB clk %d\n", ret);
		msm_geni_update_uart_error_code(port, UART_ERROR_CLK_GET_FAIL);
		return ret;
	}

	port->serial_rsc.s_ahb_clk = devm_clk_get(dev->parent, "s-ahb");
	if (IS_ERR(port->serial_rsc.s_ahb_clk)) {
		ret = PTR_ERR(port->serial_rsc.s_ahb_clk);
				     ...
				     struct msm_geni_serial_port *port)
{
	...
	struct uart_port *uport = &port->uport;
	struct resource *res;
	bool is_console = port->is_console;
	u32 wake_char = 0;
	unsigned char __iomem *membase = uport->membase;

	port->wrapper_dev = pdev->dev.parent;
	port->serial_rsc.wrapper_dev = pdev->dev.parent;
	port->serial_rsc.ctrl_dev = &pdev->dev;
	...
	port->se.clk = devm_clk_get(&pdev->dev, "se-clk");
	...
	if (!is_console)
		ret = geni_se_common_resources_init(&port->se, UART_CORE2X_VOTE,
						    APPS_PROC_TO_QUP_VOTE,
						    (DEFAULT_SE_CLK * DEFAULT_BUS_WIDTH));
	else
		ret = geni_se_common_resources_init(&port->se, GENI_DEFAULT_BW,
						    GENI_DEFAULT_BW, GENI_DEFAULT_BW);
	if (ret) {
		msm_geni_update_uart_error_code(port, UART_ERROR_SE_RESOURCES_INIT_FAIL);
		return ret;
	}

	/* RUMI specific */
	port->rumi_platform = of_property_read_bool(pdev->dev.of_node, "qcom,rumi_platform");

	if (of_property_read_u32(pdev->dev.of_node, "qcom,wakeup-byte", &wake_char)) {
		dev_dbg(&pdev->dev, "No Wakeup byte specified\n");
	} else {
		port->wakeup_byte = (u8)wake_char;
		dev_info(&pdev->dev, "Wakeup byte 0x%x\n", port->wakeup_byte);
	}

	ret = msm_geni_serial_get_clk(pdev, port);
	if (ret)
		return ret;
	...
	port->se.base = membase;
	uport->membase = membase;

	ret = msm_geni_serial_get_irq_pinctrl(pdev, port);
	if (ret)
		return ret;

	if (!is_console) {
		port->geni_wake = wakeup_source_register(uport->dev, dev_name(&pdev->dev));
		if (!port->geni_wake) {
...
static int
msm_geni_serial_port_init(struct platform_device *pdev, struct msm_geni_serial_port *port)
{
	int ret = 0;
	struct uart_port *uport = &port->uport;
	unsigned char __iomem *membase = uport->membase;
	...
	init_completion(&port->m_cmd_timeout);
	init_completion(&port->s_cmd_timeout);
	init_completion(&port->xfer);
	init_completion(&port->tx_xfer);

	init_completion(&port->wakeup_comp);
	...
	if (port->is_console) {
		...
		port->rx_fifo = devm_kzalloc(uport->dev, sizeof(u32), GFP_KERNEL);
		if (!port->rx_fifo)
			return -ENOMEM;
	} else {
		port->handle_rx = handle_rx_hs;
		port->rx_fifo = devm_kzalloc(uport->dev, (port->rx_fifo_depth * sizeof(u32)),
					     GFP_KERNEL);
		if (!port->rx_fifo)
			return -ENOMEM;
	}
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		if (drv->cons) {
			...
			if (uart_line_id >= (1 << GENI_UART_NR_PORTS) - 1) {
				dev_err(&pdev->dev, "All UART ports already initialized\n");
				return -ENODEV;
			}
	...
	if (drv->cons && !con_enabled) {
		...
		if (ret)
			dev_err(&pdev->dev, "failed to set pinctrl state to sleep %d\n", ret);
		platform_set_drvdata(pdev, port);
		return 0;
	}
	...
	ret = msm_geni_serial_read_dtsi(pdev, port);
	if (ret)
		goto exit_geni_serial_probe;
...
exit_geni_serial_probe:
	...
	UART_LOG_DBG(port->ipc_log_misc, &pdev->dev, "%s: ret:%d\n", __func__, ret);
	return ret;
}
...
{
	struct msm_geni_serial_port *port = platform_get_drvdata(pdev);
	struct uart_driver *drv = (struct uart_driver *)port->uport.private_data;
...
static int msm_geni_serial_runtime_suspend(struct device *dev)
{
	...
	int ret = 0, count = 0;
	unsigned long long start_time;
	u32 geni_status = geni_read_reg(port->uport.membase, SE_GENI_STATUS);
...
static int msm_geni_serial_sys_resume(struct device *dev)
{
	...
	if (port->is_console && !con_enabled) {
		return 0;
	} else if ((uart_console(uport) && console_suspend_enabled && uport->suspended) ||
		   port->pm_auto_suspend_disable) {
		IPC_LOG_MSG(port->console_log, "%s start %d\n", __func__, true);
		uart_resume_port((struct uart_driver *)uport->private_data, uport);
